...
import { ContactLensMetrics } from './types';

@Component({
  selector: 'app-contact-lens-orders',
  standalone: true,
...
export class ContactLensOrdersComponent implements OnInit {
  metrics: ContactLensMetrics = {
    entered: 3,
    submitted: 0,
    inProcess: 2,
    shipped: 1
  ...
  async fetchMetrics() {
    try {
      ...
      console.error('Failed to fetch contact lens metrics:', error);
    }
  }

  viewDashboard() {
    this.router.navigate(['/contact-lens-orders']);
  }
}